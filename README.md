# <div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=220&section=header&text=Vaishnavi%20Khadangle&fontSize=45&fontColor=ffffff&animation=twinkling&fontAlignY=35&desc=Embedded%20Systems%20|%20Full%20Stack%20Developer%20|%20Machine%20Learning%20Enthusiast&descAlignY=58" width="100%"/>

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono\&size=24\&duration=3000\&pause=1000\&color=8A2BE2\&center=true\&vCenter=true\&width=800\&lines=Electronics+%26+Telecommunication+Engineer;Embedded+Systems+Developer;STM32+%7C+ESP32+%7C+IoT+Developer;Full+Stack+Web+Developer;Machine+Learning+Enthusiast;Always+Learning+New+Technologies+🚀)](https://git.io/typing-svg)

![](https://komarev.com/ghpvc/?username=YOUR_USERNAME\&label=Profile+Views\&style=for-the-badge\&color=blueviolet)

</div>

---

# 👋 Hello, I'm Vaishnavi Khadangale

### Embedded Systems Engineer | Full Stack Developer | IoT Innovator | Machine Learning Enthusiast

---

# 👩‍💻 About Me

```cpp
class Vaishnavi {

public:

    string role = "B.Tech Electronics & Telecommunication Student";

    string college = "MIT Academy of Engineering, Alandi";

    string location = "Pune, Maharashtra, India";

    vector<string> expertise = {

        "Full Stack Web Development",
        "Embedded Systems",
        "STM32 Development",
        "ESP32 Development",
        "IoT",
        "Python"

    };

    vector<string> currently_learning = {

        "Machine Learning",
        "Deep Learning",
        "Computer Vision",
        "AWS Cloud"

    };

    vector<string> interests = {

        "Artificial Intelligence",
        "Embedded AI",
        "Industrial IoT",
        "Backend Development",
        "Cloud Computing"

    };

    string motto = "Learn • Build • Innovate 🚀";

};
```

* 🎓 B.Tech Electronics & Telecommunication Engineering
* 🏫 MIT Academy of Engineering (MITAOE), Alandi
* 💻 Strong foundation in Full Stack Web Development
* 🔧 Hands-on experience with STM32, ESP32, Arduino & Embedded C
* 🌐 Passionate about Backend Development & IoT
* 🤖 Currently learning Machine Learning & AI
* 🚀 Love building smart solutions using Software + Electronics

---

# 🚀 Tech Stack

## 💻 Languages

<p align="center">

<img src="https://skillicons.dev/icons?i=python,c,cpp,js,html,css&perline=6"/>

</p>

---

## 🌐 Web Development

<p align="center">

<img src="https://skillicons.dev/icons?i=nodejs,express,flask,mysql,mongodb&perline=5"/>

</p>

---

## 🔌 Embedded Systems & Electronics

<p align="center">

<!-- STM32 -->
<img src="https://img.shields.io/badge/STM32-03234B?style=for-the-badge&logo=stmicroelectronics&logoColor=white"/>

<!-- STM32CubeMX -->
<img src="https://img.shields.io/badge/STM32CubeMX-03234B?style=for-the-badge&logo=stmicroelectronics&logoColor=white"/>

<!-- STM32CubeIDE -->
<img src="https://img.shields.io/badge/STM32CubeIDE-03234B?style=for-the-badge&logo=stmicroelectronics&logoColor=white"/>

<!-- Keil -->
<img src="https://img.shields.io/badge/Keil_uVision-3949AB?style=for-the-badge&logo=arm&logoColor=white"/>

<!-- Multisim -->
<img src="https://img.shields.io/badge/NI_Multisim-FFC107?style=for-the-badge&logo=ni&logoColor=black"/>

<!-- Arduino -->
<img src="https://img.shields.io/badge/Arduino-00979D?style=for-the-badge&logo=arduino&logoColor=white"/>

<!-- ESP32 -->
<img src="https://img.shields.io/badge/ESP32-E7352C?style=for-the-badge"/>

</p>

---

### ⚙️ Embedded Programming

- Embedded C
- GPIO
- UART
- SPI
- I2C
- PWM
- ADC
- Timers
- Interrupts
- Serial Communication
- Sensor Interfacing
- Real-Time Embedded Programming

---

### 🌐 IoT Technologies

- MQTT
- ESP32 Wi-Fi
- REST APIs
- Sensor Integration
- Embedded Firmware Development
## 🤖 AI & Machine Learning

<p align="center">

<img src="https://skillicons.dev/icons?i=tensorflow&theme=dark"/>

<img src="https://go-skill-icons.vercel.app/api/icons?i=scikitlearn"/>

<img src="https://skillicons.dev/icons?i=pandas&theme=dark"/>

<img src="https://skillicons.dev/icons?i=numpy&theme=dark"/>

</p>

* Scikit-Learn
* Pandas
* NumPy
* Matplotlib

---

## ☁️ Tools & Technologies

<p align="center">

<img src="https://skillicons.dev/icons?i=git,github,vscode,docker,linux,aws&perline=6"/>

</p>

---

# 🏆 GitHub Trophy

<p align="center">

<img src="https://github-profile-trophy.vercel.app/?username=YOUR_USERNAME&theme=tokyonight&no-frame=true&row=2&column=4"/>

</p>

---

# 📊 GitHub Statistics

<p align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=tokyonight&hide_border=true"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_USERNAME&layout=compact&theme=tokyonight&hide_border=true"/>

</p>

---

# 🔥 GitHub Streak

<p align="center">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_USERNAME&theme=tokyonight&hide_border=true"/>

</p>

---

# 📈 Contribution Graph

<p align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=YOUR_USERNAME&theme=tokyo-night&hide_border=true&area=true"/>

</p>

---

# 🚀 Featured Projects

| Project                            | Description                                      | Technologies               |
| ---------------------------------- | ------------------------------------------------ | -------------------------- |
| 🤖 AI Powered Digital Nurse        | Remote Patient Monitoring System                 | ESP32, Flask, Firebase, AI |
| 💧 Smart Water Pressure Management | IoT Dashboard for Smart Water Distribution       | Python, Flask, Chart.js    |
| 🔧 STM32 Embedded Projects         | GPIO, UART, PWM, ADC, Timers, Sensor Interfacing | STM32                      |
| 🌦 Weather Forecast Application    | Real-Time Weather Dashboard                      | Python, Flask              |
| 🔐 Encryption & Decryption Website | Secure Text Encryption Tool                      | HTML, CSS, JavaScript      |
| 🌐 Personal Portfolio              | Responsive Personal Portfolio Website            | HTML, CSS, JavaScript      |

---

# 🎯 Current Focus

```yaml
2026 Goals

✅ Master Full Stack Development

✅ Build Advanced STM32 Projects

✅ Learn Machine Learning

🔄 Deep Learning

🔄 Computer Vision

🔄 AWS Cloud

🔄 Docker

🔄 Open Source Contribution

📌 AI + IoT Projects

📌 Software Engineering Internship
```

---

# 📚 Currently Learning

* Machine Learning
* Deep Learning
* Computer Vision
* AWS Cloud
* Docker
* System Design

---

# 📊 Coding Activity

<!--START_SECTION:waka-->

Coming Soon...

<!--END_SECTION:waka-->

---

# 🐍 Contribution Snake

> Enable the GitHub Action later to generate the snake animation.

![Snake animation](https://github.com/YOUR_USERNAME/YOUR_USERNAME/blob/output/github-contribution-grid-snake.svg)

---

# 💡 Developer Quote

![](https://quotes-github-readme.vercel.app/api?type=horizontal\&theme=tokyonight)

---

# 📫 Connect With Me

<p align="center">

<a href="YOUR_LINKEDIN">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://github.com/YOUR_USERNAME">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="mailto:YOUR_EMAIL">
<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="YOUR_PORTFOLIO">
<img src="https://img.shields.io/badge/Portfolio-6E40C9?style=for-the-badge&logo=vercel&logoColor=white"/>
</a>

</p>

---

# ⚡ Fun Fact

> **"I enjoy combining Embedded Systems, IoT, Web Development, and Artificial Intelligence to create smart real-world solutions."**

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=120&section=footer&color=gradient"/>

### ⭐ Thanks for visiting my profile!

**Building Intelligent Systems with Electronics + Software + AI 🚀**

</div>
